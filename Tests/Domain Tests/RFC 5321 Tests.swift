...
struct RFC5321Tests {
    ...
    func testStandardDomain() throws {
        let domain = try RFC5321("mail.example.com")
    ...
    func testIPv4Literal() throws {
        let domain = try RFC5321("[192.168.1.1]")
    ...
    func testIPv6Literal() throws {
        let domain = try RFC5321("[2001:db8:85a3:8d3:1319:8a2e:370:7348]")
    ...
    func testEmptyAddressLiteral() throws {
        #expect(throws: RFC5321.ValidationError.emptyAddressLiteral) {
            _ = try RFC5321("[]")
        }
    }
    ...
    func testInvalidIPv4Format() throws {
        #expect(throws: RFC5321.ValidationError.invalidIPv4("256.256.256.256")) {
            _ = try RFC5321("[256.256.256.256]")
        }
    }
    
    @Test("Fails with invalid IPv6 format")
    func testInvalidIPv6Format() throws {
        #expect(throws: RFC5321.ValidationError.invalidIPv6("not:valid:ipv6")) {
            _ = try RFC5321("[not:valid:ipv6]")
        }
    }
    ...
    func testCreateFromRFC1123() throws {
        let rfc1123 = try RFC1123("mail.example.com")
        let domain = RFC5321(domain: rfc1123)
        #expect(domain.name == "mail.example.com")
        #expect(domain.isStandardDomain)
    }
    ...
    func testCreateIPv4Literal() throws {
        let domain = try RFC5321(ipv4Literal: "192.168.1.1")
        #expect(domain.name == "[192.168.1.1]")
        #expect(domain.addressLiteral == "192.168.1.1")
    }
    ...
    func testCreateIPv6Literal() throws {
        ...
        let domain = try RFC5321(ipv6Literal: ipv6)
    ...
    func testCodableStandardDomain() throws {
        let original = try RFC5321("mail.example.com")
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RFC5321.self, from: encoded)
        #expect(original == decoded)
    }
    
    @Test("Successfully encodes and decodes IPv4 literal")
    func testCodableIPv4() throws {
        let original = try RFC5321("[192.168.1.1]")
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RFC5321.self, from: encoded)
        #expect(original == decoded)
    }
