...
struct RFC1123Tests {
    ...
    func testInvalidTLDStartingWithNumber() throws {
        #expect(throws: RFC1123.ValidationError.invalidTLD("123com")) {
            _ = try RFC1123("example.123com")
        }
    }
    
    @Test("Fails with invalid TLD ending with number")
    func testInvalidTLDEndingWithNumber() throws {
        #expect(throws: RFC1123.ValidationError.invalidTLD("com123")) {
            _ = try RFC1123("example.com123")
        }
    }
    
    @Test("Fails with invalid label containing special characters")
    func testInvalidLabelSpecialChars() throws {
        #expect(throws: RFC1123.ValidationError.invalidLabel("host@name")) {
            _ = try RFC1123("host@name.com")
        }
    }
    
    @Test("Successfully gets TLD")
    func testTLD() throws {
        let host = try RFC1123("example.com")
    ...
    func testIsSubdomain() throws {
        let parent = try RFC1123("example.com")
        let child = try RFC1123("host.example.com")
    ...
    func testRootInitializer() throws {
        let host = try RFC1123.root("example", "com")
        #expect(host.name == "example.com")
    }
    
    @Test("Successfully creates host from subdomain components")
    func testSubdomainInitializer() throws {
        let host = try RFC1123.subdomain("com", "example", "host")
        #expect(host.name == "host.example.com")
    }
    
    @Test("Successfully encodes and decodes")
    func testCodable() throws {
        let original = try RFC1123("example.com")
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RFC1123.self, from: encoded)
