...
struct RFC1035Tests {
    ...
    func testEmptyDomain() throws {
        #expect(throws: RFC1035.ValidationError.empty) {
            _ = try RFC1035("")
        }
    }
    ...
    func testTooManyLabels() throws {
        ...
        #expect(throws: RFC1035.ValidationError.tooManyLabels) {
            _ = try RFC1035(longDomain)
        }
    }
    ...
    func testTooLongDomain() throws {
        ...
        #expect(throws: RFC1035.ValidationError.tooLong(319)) {
            _ = try RFC1035(longDomain)
        }
    }
    ...
    func testInvalidLabelStartingWithHyphen() throws {
        #expect(throws: RFC1035.ValidationError.invalidLabel("-example")) {
            _ = try RFC1035("-example.com")
        }
    }
    ...
    func testIsSubdomain() throws {
        let parent = try RFC1035("example.com")
        let child = try RFC1035("sub.example.com")
    ...
    func testRootInitializer() throws {
        let domain = try RFC1035.root("example", "com")
        #expect(domain.name == "example.com")
    }
    
    @Test("Successfully creates domain from subdomain components")
    func testSubdomainInitializer() throws {
        let domain = try RFC1035.subdomain("com", "example", "sub")
        #expect(domain.name == "sub.example.com")
    }
    
    @Test("Successfully encodes and decodes")
    func testCodable() throws {
        let original = try RFC1035("example.com")
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RFC1035.self, from: encoded)
